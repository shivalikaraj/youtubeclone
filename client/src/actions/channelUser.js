...
export const toggleRestriction = (channelId, isRestricted) => async (dispatch) => {
    try {
        const {data} = await api.toggleRestriction(channelId, {isRestricted})
        dispatch({type: 'TOGGLE_RESTRICTION', payload: data.message})
    } catch (error) {
        console.log(error)
    }
}